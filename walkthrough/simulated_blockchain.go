...
func simulatedBlockchain(wg *sync.WaitGroup, dispute bool) {
	...
	fmt.Printf("\nStart of Walkthrough with simulated blockchain connection")
	fmt.Printf("\nGreen is from alice (sender),\nYellow is from Bob(receiver) \n\n")

	wg2 := &sync.WaitGroup{}
	wg2.Add(2)
	simulatedBlockchainAlice(aliceColor, wg2, dispute)
	wg2.Wait()

	wg.Done()
	fmt.Printf("\nEnd of Walkthrough with simulated blockchain connection\n\n")
}
func simulatedBlockchainAlice(printer *color.Color, wg *sync.WaitGroup, dispute bool) {
	defer wg.Done()
	...
	doneBobWalkthroughInit, err := simulatedBlockchainBob(&bcInst, bobColor, wg)
	if err != nil {
		_, _ = printer.Println("Initializing bob connection error -", err)
		return
	}
	if !doneBobWalkthroughInit {
		_, _ = printer.Println("Initializing bob connection failed without error")
		return
	}
	...
	mscEventInitializing := <-eventsChan.MSCInitializingChan
	...
	isValid, reason = newConnToBob.ValidateIncomingState(vpcStateSigned03)
	if !isValid {
		_, _ = printer.Printf("\nIncoming Vpc state invalid - %v\n", reason)
		return
	}
	...
	isValid, reason = newConnToBob.ValidateIncomingState(vpcStateSigned05)
	if !isValid {
		_, _ = printer.Printf("\nIncoming Vpc state invalid - %v\n", reason)
		return
	}
	...
	isValid, reason = newConnToBob.ValidateIncomingState(vpcStateSigned07)
	if !isValid {
		_, _ = printer.Printf("\nIncoming Vpc state invalid - %v\n", reason)
		return
	}
	...
	isValid, reason = newConnToBob.ValidateIncomingState(vpcStateSigned10)
	if !isValid {
		_, _ = printer.Printf("\nIncoming Vpc state invalid - %v\n", reason)
		return
	}
...
func simulatedBlockchainBob(bcInst *blockchain.Instance,
	printer *color.Color, wg *sync.WaitGroup) (done bool, err error) {

	//Initialize bob's bcInstance
	bcInst2 := blockchain.NewInstance(bcInst.Conn, bobID)
	...
	go func(bcInst2 *blockchain.Instance,
		...
		if err != nil {
			_, _ = printer.Printf("\nContract Address Read (vpc) error - %v\n", err)
			return
		}
		...
		if err != nil {
			_, _ = printer.Printf("\nContract Address Read (ms contract) error - %v\n", err)
			return
		}
		...
		bcInst2.MSContractInst, _ = contract.NewMSContract(bcInst2.MSContractAddr().Address, bcInst2.Conn)
		bcInst2.VPCInst, _ = contract.NewVPC(bcInst2.VPCAddr().Address, bcInst2.Conn)
		...
		isValid, reason := newConnFromAlice.ValidateIncomingState(vpcStateSigned01)
		if !isValid {
			_, _ = printer.Printf("\nIncoming Vpc state invalid - %v\n", reason)
			return
		}
		...
		isValid, reason = newConnFromAlice.ValidateIncomingState(vpcStateSigned04)
		if !isValid {
			_, _ = printer.Printf("\nIncoming Vpc state invalid - %v\n", reason)
			return
		}
		...
		isValid, reason = newConnFromAlice.ValidateIncomingState(vpcStateSigned06)
		if !isValid {
			_, _ = printer.Printf("\nIncoming Vpc state invalid - %v\n", reason)
			return
		}
		...
		isValid, reason = newConnFromAlice.ValidateIncomingState(vpcStateSigned08)
		if !isValid {
			_, _ = printer.Printf("\nIncoming Vpc state invalid - %v\n", reason)
			return
		}
		...
		isValid, reason = newConnFromAlice.ValidateIncomingState(vpcStateSigned09)
		if !isValid {
			_, _ = printer.Printf("\nIncoming Vpc state invalid - %v\n", reason)
			return
		}
	...
	}(&bcInst2, incomingConnChan, listener, wg)
